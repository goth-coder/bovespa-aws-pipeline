...
def get_execution_id(context: Any) -> str:
    """
    Extrai o execution ID do contexto de forma segura.
    Funciona tanto com contexto real da AWS quanto com mock para testes.
    """
    if not context:
        return 'local-test'
    
    # Contexto real da AWS Lambda (tem atributo aws_request_id)
    if hasattr(context, 'aws_request_id'):
        return context.aws_request_id
    
    # Contexto mock para testes (dicionário)
    if isinstance(context, dict):
        return context.get('aws_request_id', 'local-test')
    ...
    try:
        ...
        result = {
            ...
            'body': json.dumps({
                ...
                'executionId': get_execution_id(context),
                'results': {
